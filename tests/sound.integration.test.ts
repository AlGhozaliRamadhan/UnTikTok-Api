// tests/sound.integration.test.ts
// Vitest version of tests/test_sound.ts (ADR-002).
// Gated by CI_NETWORK — skipped by default to avoid spinning up Playwright.

import { describe, it, expect } from "vitest";
import { TikTokApi } from "../src";

const msToken = process.env.ms_token ?? undefined;
const headless = (process.env.headless ?? "true").toLowerCase() === "true";
const browser = (process.env.TIKTOK_BROWSER as "chromium" | "firefox" | "webkit") ?? "chromium";

describe.skipIf(!process.env.CI_NETWORK || !process.env.ms_token)("integration: sound.info + sound.videos", () => {
  it("returns info and at least 1 video for a known sound", async () => {
    const api = new TikTokApi();
    try {
      await api.createSessions({
        msTokens: msToken ? [msToken] : null,
        numSessions: 1,
        sleepAfter: 3,
        browser,
        headless,
      });

      const sound = api.sound({ id: "7016547803243022337" });
      const info = await sound.info();
      expect(info).not.toBeNull();
      expect(sound.id).toBeDefined();
      expect(sound.title).toBeDefined();

      let videoCount = 0;
      for await (const video of sound.videos(30)) {
        expect(video.id).toBeDefined();
        videoCount++;
      }
      expect(videoCount).toBeGreaterThan(0);
    } finally {
      await api.closeSessions();
    }
  });
});
